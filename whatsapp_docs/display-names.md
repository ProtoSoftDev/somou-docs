...
After updating your display name via [WhatsApp Manager](#update-display-name-via-whatsapp-manager) or [API](#update-display-name-via-api), you must re-register your phone number to apply the new display name with WhatsApp servers.
...
1. Call `POST /&lt;PHONE_NUMBER_ID&gt;/register` to re-register the phone number. See [Register a Business Phone Number](https://developers.facebook.com/documentation/business-messaging/whatsapp/business-phone-numbers/registration#register) for endpoint details.
