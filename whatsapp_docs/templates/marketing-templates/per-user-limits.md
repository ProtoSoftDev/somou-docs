...
WhatsApp does not currently deliver marketing template messages to WhatsApp users with United States phone numbers (numbers composed of a +1 dialing code and a US area code). Attempting to send a template message to a WhatsApp user with a US phone number after this date will result in an error.
...
To ensure your campaigns reach your audience, wait at least 24 hours before attempting to resend a marketing template message to a user that has reached their limit. Resending earlier than this will likely result in additional error responses and can reduce the accuracy of campaign delivery reporting.
